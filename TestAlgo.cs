...
public class TestAlgo : IAlgorithm
{
    ...
    public TestAlgo(string inputPath, string outputPath, IComparer<string> comparer)
    {
    ...
    private IEnumerable<IStep> FindRuns(int inputFileIndex, int runsLength)
    {
    ...
    public IEnumerable<IStep> BreakFileIntoRuns(int inputFileIndex, int runsLength)
    {
        var currFileIndex = 0;
        var currRunLength = 0;
        ...
        while (ReadLine(inputFile.BaseStream, true) is string line)
        {
            ...
            if (currRunLength == runsLength)
            {
                ...
                // files[currFileIndex].Item2.WriteLine(line);
                currFileIndex = (currFileIndex + 1) % _files.Length;
                if (currFileIndex == inputFileIndex)
                {
                    currFileIndex = (currFileIndex + 1) % _files.Length;
    ...
    private IEnumerable<IStep> Sort(int inputFileIndex, int runsLength)
    {
        if (_lines == 0) new List<IStep>(BreakFileIntoRuns(inputFileIndex, runsLength));
        while (_lines > runsLength)
        {
            yield return new ResetRunColorsStep();
            foreach (var step in FindRuns(inputFileIndex, runsLength)) yield return step;
            foreach (var step in BreakFileIntoRuns(inputFileIndex, runsLength)) yield return step;
            for (int i = 1; i < _files.Length; i++)
            {
                yield return new ResetRunColorsStep();
                foreach (var step in FindRuns(i, runsLength)) yield return step;
            }
            foreach (var step in MergeFilesSingleRun(inputFileIndex, runsLength)) yield return step;
            runsLength *= 2;
        }
    ...
    private IEnumerable<IStep> MergeFilesSingleRun(int outputFileIndex, int runsLength)
    {
        ...
        while (CanReadAtLeastOneFile(exceptInputFile: true))
        {
            ...
            while (remainingRuns.Sum() < (runsLength * (_files.Length - 1)))
            {
    ...
    public IEnumerable<IStep> Run()
    {
        ...
        // foreach (var run in FindRuns(0, 1))
        // {
        //     yield return run;
        //     test++;
        // }
        // test += 0;
        // foreach (var step in BreakFileIntoRuns(0, 1))
        // {
        //     yield return step;
        // }
        // foreach (var step in BreakFileIntoRuns(0, 2))
        // {
        //     yield return step;
        // }
        // foreach (var step in BreakFileIntoRuns(0, 4))
        // {
        //     yield return step;
        // }
        // foreach (var step in BreakFileIntoRuns(0, 8))
        // {
        //     yield return step;
        // }
        // foreach (var step in BreakFileIntoRuns(0, 16))
        // {
        //     yield return step;
        // }
        foreach (var step in Sort(0, 1))
        {
            yield return step;
        }
